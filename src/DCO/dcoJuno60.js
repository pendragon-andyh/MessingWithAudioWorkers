...
export class DcoJuno60 extends DcoCore {
	...
	process(lfoValue, envValue, bendValue) {
		...
		let x=this.currentPhase-this.pwmComparisonLevel;
		...
		return (newSawOutput*this.sawLevel.getValue())+
			(newPulseOutput*this.pulseLevel.getValue())+
			(newSubOutput*this.subLevel.getValue());
	}
